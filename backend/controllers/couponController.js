...
exports.couponValidate = catchAsync(async (req, res, next) => {
  ...
  const coupon = await Coupon.aggregate([
      ...
      $addFields: {
        finalTotal: {
          $cond: [
            ...
            { $subtract: [cartItemsTotalAmount, { $min: [{ $multiply: [cartItemsTotalAmount, { $divide: ["$discount", 100] }] }, "$maxDiscount"] }] },
    ...
    { $project: { _id: 0, subTitle: 1, couponName: 1, details: 1, minAmount: 1, finalTotal: 1, message: 1 } },
  ]);
