...
exports.generateFoodAI = catchAsync(async (req, res) => {
  ...
  if (!name || !category || !spiceLevel || !price) {
    return res.status(400).json({ success: false, message: "name, category, spiceLevel and price are required" });
  }
  const aiData = await aiService.generateDishDescription({ name, category, spiceLevel, price });
...
exports.generateAndSaveFoodAI = catchAsync(async (req, res) => {
  ...
  const aiData = await aiService.generateDishDescription({ name: food.name, category: food.category || "Veg", spiceLevel: food.spiceLevel || "Medium", price: food.price });
...
exports.analyzeRestaurantReviews = catchAsync(async (req, res) => {
  try {
    ...
    if (!restaurant.reviews.length) return res.status(400).json({ message: "No reviews to analyze" });
